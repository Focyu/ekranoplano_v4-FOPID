...
x_nom(1)  = 15.0;        % u [m/s] — velocidad crucero
x_nom(3)  = 0;              % w [m/s]
...
m      = 1.5;          % <-- masa del ekranoplano AirFish 8 en kg (AJUSTA a tu valor real)
g      = 9.81;         % aceleracion de la gravedad [m/s^2]
h_max  = 0.5;          % altura limite de la zona de flotacion [m]

k_boya = 20*m*g / h_max;              % rigidez de la fuerza restauradora [N/m]
zeta   = 0.7;                         % factor de amortiguamiento (0.7 = buen compromiso)
c_boya = 2*sqrt(k_boya*m)*zeta;       % coeficiente de amortiguamiento [N*s/m]

%% Simulation
sim = sim('FOpid_control.slx');
S = sim.states;
t = sim.tout;
C = sim.control_out; 
LD_ratio = sim.LD;
